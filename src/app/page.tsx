...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Phần Danh Mục (Carousel giống sản phẩm) */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-gray-600">
              Khám phá các danh mục sản phẩm được chọn lọc kỹ lưỡng của chúng tôi
            </p>
          </div>
          ...
          {loading ? (
            <div className="text-center text-gray-500 py-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-8">
                {[...Array(categoriesPerPage)].map((_, index) => (
                  <div key={index} className="aspect-w-4 aspect-h-3 rounded-lg bg-gray-200 animate-pulse"></div>
                ))}
              </div>
            </div>
          ) : error ? (
            ...
            <div className="relative">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 transition-transform duration-300 ease-in-out">
                <div
                  className="flex"
                  style={{
                    transform: `translateX(-${currentCategoryIndex * (100 / categoriesPerPage)}%)`,
                    width: `${(categories.length / categoriesPerPage) * 100}%`,
                  }}
                >
                  {categories.map((category, index) => (
                    <Link
                      key={category.id}
                      href={`/category/${category.id}`} // Sử dụng id thay vì slug
                      className="group relative overflow-hidden rounded-lg shadow-lg hover:shadow-xl transition-shadow aspect-w-4 aspect-h-3 w-full flex-shrink-0"
                    >
                      <div
                        className={`absolute inset-0 bg-gradient-to-br from-purple-600 to-purple-800 opacity-90 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center ${
                          index % 4 === 1
              ...
              {categories.length > categoriesPerPage && (
                <>
                  <button
                    ...
                    className="absolute left-0 top-1/2 -translate-y-1/2 bg-white/70 hover:bg-white p-2 rounded-full shadow-md transition-colors z-10 hidden md:block"
                  ...
                  <button
                    ...
                    className="absolute right-0 top-1/2 -translate-y-1/2 bg-white/70 hover:bg-white p-2 rounded-full shadow-md transition-colors z-10 hidden md:block"
                  ...
                  {Array.from({
                    ...
                    <button
                      ...
                      className={`w-3 h-3 rounded-full transition-all duration-300
                        ${currentCategoryIndex === index ? "bg-purple-600 w-6" : "bg-gray-400"}`}
                ...
                {[...Array(categoriesPerPage)].map((_, index) => (
                  ...
                  >
                    <div className="text-center p-4">
                      <h3 className="text-xl font-semibold mb-2">Danh mục {index + 1}</h3>
                      <p className="text-sm opacity-90">Mô tả danh mục</p>
                    </div>
                  </div>
                ))}
              </div>
